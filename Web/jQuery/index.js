...
var outstandingBills ;
 ...
 $("#selectcustomer").autocomplete({
	...
	$('.saveorder').one("click",function(event) {  

		so.customerId=customerId;
		so.employeeId=43;
		so.entries = entries;
		so.paymentAmount = 100.00;
	...
	$.ajax({
		...
		success : function( response ){
			...
			if( response.error ) {
				...
				alert("Order Entries have been saved!");
	...
	{     
		 ...
		 for(var i=0;i<gid.length;i++)
		{
			...
			if(stotal != "0.00")
			{
			$('#reviewtable').append('<tbody id="tablebody"><tr><td align="center">'+productvalue+'</td><td align="center">'+qtyvalue+'</td><td align="center">'+uprice+'</td><td align="center">'+stotal+'</td></tr></tbody>');
